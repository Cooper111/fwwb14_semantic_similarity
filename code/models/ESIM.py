...
def fuck_esim(pretrained_embedding=config.word_embed_weights,
         maxlen=MAX_LEN,
         lstm_dim=300,
         dense_dim=300,
         dense_dropout=0.2):
    ...
    q1_c = Input(name='q1_c1', shape=(maxlen,))
    q2_c = Input(name='q2_c1', shape=(maxlen,))

    q1_w = Input(name='q1_w1', shape=(maxlen,))
    q2_w = Input(name='q2_w1', shape=(maxlen,))
    ...
    q1_encoded = encode(q1_c)
    q2_encoded = encode(q2_c)
    ...
    model = Model(inputs=[q1_c, q2_c, q1_w,q2_w,magic_input], outputs=dense)
...
def fuck_decomposable_attention(pretrained_embedding=config.word_embed_weights,
                           projection_dim=300, projection_hidden=0, projection_dropout=0.2,
                           compare_dim=500, compare_dropout=0.2,
                           dense_dim=300, dense_dropout=0.2,
                           lr=1e-3, activation='elu', maxlen=MAX_LEN):
    ...
    model = Model(inputs=[q1, q2,magic_input], outputs=dense)
    model.compile(optimizer=Adam(lr=lr), loss='binary_crossentropy',
