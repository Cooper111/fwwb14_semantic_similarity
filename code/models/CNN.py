...
def fuck_model_conv1D_(lr=0.005):
    ...
    seq1_char = Input(shape=(config.word_maxlen,),name='q1_c2')
    seq2_char = Input(shape=(config.word_maxlen,),name='q2_c2')

    seq1_word = Input(shape=(config.word_maxlen,),name='q1_w2')
    seq2_word = Input(shape=(config.word_maxlen,),name='q2_w2')
    magic_input = Input(shape=(len(config.feats),))
    ...
    #pred = Dense(1, activation='sigmoid')(x)
    #model = Model(inputs=[seq1_char, seq2_char, magic_input], outputs=pred)
    model = Model(inputs=[seq1_char, seq2_char,seq1_word,seq2_word, magic_input], outputs=x)
    # model.compile(loss='binary_crossentropy',
    #               optimizer=Adam(lr=lr),metrics = [Precision,Recall,F1,])
    print('fuck_cnn:')
    model.summary()
