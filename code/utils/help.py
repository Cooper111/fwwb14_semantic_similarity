...
def get_X_Y_from_df(data, data_augment=True, shuffer=True):
    #data=data[:200]
    data['q_cut_id']=data['q_cut_id'].map(lambda x: x[:config.char_maxlen])
    data['a_cut_id']=data['a_cut_id'].map(lambda x: x[:config.char_maxlen])
    data['q_word_id']=data['q_word_id'].map(lambda x: x[:config.word_maxlen])
    data['a_word_id']=data['a_word_id'].map(lambda x: x[:config.word_maxlen])
    ...
    if shuffer:
        data1=data.copy()
        data1['q_cut_id']=data1['q_cut_id'].map(lambda x: ss(x))
        data1['a_cut_id']=data1['a_cut_id'].map(lambda x: ss(x))
        data=data.append(data1)

    data_q1=np.array(list(data.q_cut_id.values))
    data_q2=np.array(list(data.a_cut_id.values))
    data_q1_word=np.array(list(data['q_word_id'].values))
    data_q2_word=np.array(list(data['a_word_id'].values))
    ...
    if data_augment:
        ...
        X=[np.concatenate([data_q1_word, data_q2_word], 0),
             np.concatenate([data_q2_word, data_q1_word], 0),
             np.concatenate([magic_feat, magic_feat], 0)
             ]
        #X=[data_q2_word, data_q2_word, magic_feat]
    

    else:
        data_label=data_label
        X=[data_q1, data_q2, data_q1_word, data_q2_word, magic_feat]
    Y=data_label
