...
def data_2id(data):
    ...
    data['q_cut_id'] = data['q_cut'].map(lambda x: word2id(x, vocab))
    data['a_cut_id'] = data['a_cut'].map(lambda x: word2id(x, vocab))
    data['q_word_id'] = data['q_cut_word'].map(lambda x: word2id(x, vocab_word))
    data['a_word_id'] =data['a_cut_word'].map(lambda x: word2id(x, vocab_word))
    return data


feats0 = ['q_freq', 'a_freq', 'freq_mean',
...
def merge_feats(data, featdirs):
    ...
    if not os.path.exists(featdirs[2]):
        ...
        # pass
        df0 = pd.read_csv(featdirs[0], encoding='gbk')
        df1 = pd.read_csv(featdirs[1], encoding='gbk')
        df2 = pd.read_csv(featdirs[2], encoding='gbk')
